...
## Data Structure
Expects the following files in a `data/` directory (these are not included in the repo but are required to run the pipeline):
```
data/
...
## Dependencies
This is a PyTorch-heavy stack:
```bash
pip install torch torch-geometric transformers scikit-learn pandas numpy matplotlib seaborn
```

## How to use
Just run `EpiGraph_AI.ipynb` top-to-bottom. It will:
